...
public:
	//constructors::
	Tractvolsx(const bool& usefin = false) :
			opts(probtrackxOptions::getInstance()), logger(
					LogSingleton::getInstance()), init_sample(true), fibst(0), usef(
					usefin) {
	}
	~Tractvolsx() {
	}
	int get_nfibres() const {
		return nfibres;
	}
	...
	int sample_fibre(int col, int samp, const ColumnVector& dir) {
		...
		for (int fib = 0; fib < nfibres; fib++) {
			...
			if (ft > opts.fibthresh.value()) {
				...
				if (fabs(x(1) * dir(1) + x(2) * dir(2) + x(3) * dir(3))
						> 0.766) { //hard-coded 40 deg threshold
		...
		if (mode == 3) { //sample all
			...
			if (mode == 1) { //sample all>thresh
				...
				if (fibvec.size() == 0) {
					...
					double rtmp = (rand() / (double(RAND_MAX) + 1))
							* fibvec.size();
					return (fibvec[(int) floor(rtmp)]);
				}
				...
				cerr << "TRACTVOLSX::sample_fibre:Error - unknown mode = "
						<< mode << endl;
				exit(1);
			}
		}
	...
	int sample_ang_prob(const vector<float>& probs) {
		float sum = 0;
		ColumnVector cumsum(probs.size());
		cumsum = 0;
		int ind = 0;
	...
	void initialise(const string& basename, const volume<float>& mask) {
		...
		// Can read in coordinates using either spherical.h
		// or using read_ascii_matrix
		/*
		cout << "InitDir value: " << opts.init_dir.value() << endl;
		Spherical S = Spherical(opts.init_dir.value());
		S.get_angles();
		vector<double> testing = S.point(0);
		cout << "Vector length: " << testing.size() << endl;
		cout << "Azimuthal: " << S.theta(0) << endl;
		cout << "Polar: " << S.phi(0) << endl;

		Matrix angles = read_ascii_matrix(opts.init_dir.value());
		cout << "Angles Columns: " << angles.Ncols() << endl;
		cout << "Angles Rows: " << angles. Nrows() << endl;
		*/
		...
		if (fsl_imageexists(basename + "_thsamples")) {
			...
			while (fib_existed) {
				if (fsl_imageexists(
						basename + "_th" + num2str(fib) + "samples")) {
					cout << fib << "_1" << endl;
					read_volume4D(tmpvol,
							basename + "_th" + num2str(fib) + "samples");
					tmpmat = tmpvol.matrix(mask);
					thsamples.push_back(tmpmat);
					cout << fib << "_2" << endl;
					read_volume4D(tmpvol,
							basename + "_ph" + num2str(fib) + "samples");
					tmpmat = tmpvol.matrix(mask);
					phsamples.push_back(tmpmat);
					cout << fib << "_3" << endl;
					read_volume4D(tmpvol,
							basename + "_f" + num2str(fib) + "samples");
					tmpmat = tmpvol.matrix(mask);
					fsamples.push_back(tmpmat);
					fib++;
				} else {
			...
			if (fib == 1) {
				cerr
						<< "Could not find samples to load. Exit without doing anything"
						<< endl;
				exit(1);
			}
			lut_vol2mat = tmpvol.vol2matrixkey(mask);
	...
	ColumnVector sample(const float& x, const float& y, const float&z,
			...
			const int& sample_fib, int& sampled_fib, int& newx, int& newy,
			int& newz) {
		...
		int samp = (int) MISCMATHS::round(
				(float) rand() / float(RAND_MAX) * (float) (nsamples - 1)) + 1;
		...
		if (nfibres > 1) {	//more than 1 fibre
			if (init_sample) {//go for the specified fibre on the first jump or generate at random
				...
				if (sample_fib > 0) { // pick specified fibre
					...
					if ((fabs(prefer_x) + fabs(prefer_y) + fabs(prefer_z))
							== 0) {
						prefer_x = r_x;
						prefer_y = r_y;
						prefer_z = r_z;
					...
					if (locrule == 1) {
						...
						for (int fib = 0; fib < nfibres; fib++) {
							if (fsamples[fib](samp, col)
									> opts.fibthresh.value()) {
