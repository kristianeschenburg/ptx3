...
void MatCell_cmpr::decode(int64_t incode, int& nsamples, int& fibcnt1,
		int& fibcnt2, float& length_tot) const {
	//undo the coding for incode
	//code = two32*fibre_count + mult*mult*fibre_prop1 + mult*fibre_prop2 + length_val;
	int64_t fibre_count, fibre_prop1, fibre_prop2, length_val, two32 = (1LL
			<< 32), mult = 1001, multmult = mult * mult;
	...
	fibcnt1 = (int) MISCMATHS::round(
			(float) (fibre_prop1 * 0.001 * fibre_count));
	fibcnt2 = (int) MISCMATHS::round(
			(float) (fibre_prop2 * 0.001 * fibre_count));
	length_tot = float(length_val * fibre_count);
	nsamples = (int) fibre_count;
}

void MatCell_cmpr::encode(const int nsamples, const int fibcnt1,
		const int fibcnt2, const float length_tot) {
	...
	fibre_prop1 =
			(int64_t) MIN(
					(int64_t )(MISCMATHS::round(
							float(fibcnt1) / float(nsamples) * 1000)), 1000);
	fibre_prop2 =
			(int64_t) MIN(
					(int64_t )(MISCMATHS::round(
							float(fibcnt2) / float(nsamples) * 1000)), 1000);
	fibre_prop2 = (int64_t) MIN(fibre_prop2, (int64_t )(1000 - fibre_prop1)); //Avoid sum over 1000 due to rounding errors
	length_val = (int64_t) MIN(
			(int64_t )(MISCMATHS::round(
					(nsamples != 0 ? length_tot / float(nsamples) : 0.0))),
			1000);
	code2 = two32 * fibre_count + mult * mult * fibre_prop1 + mult * fibre_prop2
			+ length_val;
}
...
void MatCell_cmpr::add_n(int64_t newcode2) {
	int nsamples, fibcnt1, fibcnt2;
	float length_tot;
	int Newnsamples, Newfibcnt1, Newfibcnt2;
	float Newlength_tot;
...
SpMat_HCP::SpMat_HCP(unsigned int m, unsigned int n, const string& basename) :
	...
	try {
		...
		throw SpMatHCPException(
				"SpMat_HCP::SpMat_HCP(unsigned int m, unsigned int n,const string& basename): cannot open file1 for reading");
	}
	int nrows, ncols;
	fs1 >> nrows;
	fs1 >> ncols;
	...
	if (nrows != (int) Nrows() || ncols != (int) Ncols()) {
		cerr
				<< "SpMat_HCP::SpMat_HCP(unsigned int m, unsigned int n,const string& basename): Incompatible matrix dimensions"
				<< endl;
		exit(1);
	}
	...
	try {
		fs2.open(file2.c_str(), ios::in | ios::binary);
	} catch (...) {
		throw SpMatHCPException(
				"SpMat_HCP::SpMat_HCP(unsigned int m, unsigned int n,const string& basename): cannot open file2 for reading");
	}
	...
	try {
		fs3.open(file3.c_str(), ios::in | ios::binary);
	} catch (...) {
		throw SpMatHCPException(
				"SpMat_HCP::SpMat_HCP(unsigned int m, unsigned int n,const string& basename): cannot open file3 for reading");
	}
	int64_t code1, code2;
...
int SpMat_HCP::SaveTrajFile(const string& basename) const {
	...
	try {
		...
		throw SpMatHCPException(
				"SpMat_HCP::SaveTrajFile(const string& basename): cannot open file1 for writing");
	}
	fs1 << Nrows() << endl;
	fs1 << Ncols() << endl;
	fs1.close();
	...
	// SECOND FILE (binary file of lengths)
	try {
		fs2.open(file2.c_str(), ios::out | ios::binary);
	} catch (...) {
		throw SpMatHCPException(
				"SpMat_HCP::SaveTrajFile(const string& basename): cannot open file2 for writing");
	}
	...
	// THIRD FILE (binary file of contents, integer coded)
	try {
		fs3.open(file3.c_str(), ios::out | ios::binary);
	} catch (...) {
		throw SpMatHCPException(
				"SpMat_HCP::SaveTrajFile(const string& basename): cannot open file3 for writing");
	}
	int64_t code1, code2;
...
void read_ascii_files(const string& filename, vector<string>& content) {
	...
	if (fs) {
		...
		cerr << "TRACT::read_ascii_files: " << filename << " does not exist"
				<< endl;
		exit(0);
	}
}
...
bool compare_Vertices(const pair<int, infoVertex> &x,
		const pair<int, infoVertex> &y) {
	if (x.first < y.first)
		return true;
	if (x.first > y.first)
		return false;
	if (x.second.mesh < y.second.mesh)
		return true;
	if (x.second.mesh > y.second.mesh)
		return false;
	if (x.second.triangle < y.second.triangle)
		return true;
	if (x.second.triangle > y.second.triangle)
		return false;
...
void make_unique(vector<pair<int, infoVertex> >&x) {
	...
	for (unsigned int i = 0; i < x.size(); i++) {
		if (i > 0) {
			if (x[i].first == x[i - 1].first
					&& x[i].second.mesh == x[i - 1].second.mesh
					&& x[i].second.triangle == x[i - 1].second.triangle)
				continue;
		}
		xx.push_back(x[i]);
	}
...
Streamliner::Streamliner(const CSV& seeds) :
	...
	// particle
	m_part.initialise(0, 0, 0, 0, 0, 0, opts.steplength.value(), m_mask.xdim(),
			m_mask.ydim(), m_mask.zdim(), false);
	...
	if (opts.seeds_to_dti.value() != "") {
		if (!fsl_imageexists(opts.seeds_to_dti.value())) { //presumably ascii file provided
			...
			if (opts.dti_to_seeds.value() == "") {
				cerr << "TRACT::Streamliner:: DTI -> Seeds transform needed"
						<< endl;
				exit(1);
			}
...
void Streamliner::rotdir(const ColumnVector& dir, ColumnVector& rotdir,
	...
	if (!m_IsNonlinXfm) {
		...
		xyz_seeds << x << y << z;
		xyz_dti = NewimageCoord2NewimageCoord(m_DTI_to_Seeds_warp, false,
				m_seeds.get_refvol(), m_mask, xyz_seeds);
		...
		Jw << m_jacx(x, y, z, 0) << m_jacx(x, y, z, 1) << m_jacx(x, y, z, 2)
				<< m_jacy(x, y, z, 0) << m_jacy(x, y, z, 1)
				<< m_jacy(x, y, z, 2) << m_jacz(x, y, z, 0)
				<< m_jacz(x, y, z, 1) << m_jacz(x, y, z, 2);
...
int Streamliner::streamline(const float& x_init, const float& y_init,
		const float& z_init, const ColumnVector& dim_seeds, const int& fibst) {
	...
	// get seed coordinates in dti space
	if (!m_IsNonlinXfm)
		xyz_dti = vox_to_vox(xyz_seeds, dim_seeds, vols.dimensions(),
				m_Seeds_to_DTI);
	else {
		xyz_dti = NewimageCoord2NewimageCoord(m_DTI_to_Seeds_warp, false,
				m_seeds.get_refvol(), m_mask, xyz_seeds);
	...
	// N-Steps is max number of steps in fiber
	for (int it = 1; it <= opts.nsteps.value() / 2; it++) {

		// tracking mask is 1 at this current voxel
		if ((m_mask(x_p, y_p, z_p) != 0)) {
			...
			if (opts.loopcheck.value()) {
				...
				if (m_part.rx() * oldrx + m_part.ry() * oldry
						+ m_part.rz() * oldrz < 0) {
					break;
				}
			...
			if (cnt >= 0) {
				if (!m_IsNonlinXfm)
					xyz_seeds = vox_to_vox(xyz_dti, vols.dimensions(),
							dim_seeds, m_DTI_to_Seeds);
				else {
					xyz_seeds = NewimageCoord2NewimageCoord(m_Seeds_to_DTI_warp,
							false, m_mask, m_seeds.get_refvol(), xyz_dti);
				}
			}
			...
			if (cnt > 0 && m_surfexists) {
				float line[2][3] = { { m_path[cnt - 1](1), m_path[cnt - 1](2),
						m_path[cnt - 1](3) }, { m_path[cnt](1), m_path[cnt](2),
						m_path[cnt](3) } };
				m_seeds.line_crossed_voxels(line, crossedvox); //crossed in voxels
				m_crossedvox.push_back(crossedvox);
			}

			// check if streamline has crossed into the exclusion mask
			if (opts.rubbishfile.value() != "") {
				if (cnt > 0) {
					if (m_rubbish.has_crossed(m_path[cnt - 1], m_path[cnt],
						...
						for (unsigned int i = 0;
								i < m_way_passed_flags_updated.size(); i++) { // undo updated in this part
					...
					if (m_rubbish.has_crossed(m_path[cnt], m_path[cnt],
							crossedvox)) {
						rubbish_passed = 1;
						for (unsigned int i = 0;
								i < m_way_passed_flags_updated.size(); i++) { // undo updated in this part
			...
			if (m_way_passed_flags.size() > 0) {
				if (cnt > 0) {
					waycrossed.clear();
					m_waymasks.has_crossed_roi(m_path[cnt - 1], m_path[cnt],
							crossedvox, waycrossed);
					...
					if (opts.wayorder.value() && wayorder) {
						for (unsigned int wm = 0; wm < waycrossed.size();
								wm++) {
							for (int wm2 = 0; wm2 < waycrossed[wm]; wm2++) {
								if (m_way_passed_flags[wm2] == 0) {
									wayorder = false;
									break;
								}
							}
			...
			if (opts.wtstopfiles.value() != "" && cnt > 0) {
				wtstopcrossed.clear();
				m_wtstopmasks.has_crossed_roi(m_path[cnt - 1], m_path[cnt],
						crossedvox, wtstopcrossed);

				for (unsigned int r = 0; r < wtstop_flags.size(); r++) {
					bool crossed = false;
					for (unsigned int wm = 0; wm < wtstopcrossed.size(); wm++) {
						if (r == (unsigned int) wtstopcrossed[wm])
							crossed = true;
					}

					if ((wtstop_flags[r] == 0) && (!crossed))
						wtstop_flags[r] = 1; // going outside the roi, but we started inside
					else if ((wtstop_flags[r] == 1) && (crossed))
						wtstop_flags[r] = 2; 	// going into the roi
					else if ((wtstop_flags[r] == 2)
							&& (!crossed)
									& (m_wtstopmasks.get_roitype(r) == VOLUME)) { // going outside the roi, stop, works for volumes
						wtstop = true;
					} else if ((wtstop_flags[r] == 2)
							&& (crossed)
									& (m_wtstopmasks.get_roitype(r) == SURFACE)) { // second time that a surface is crossed, stop
						wtstop = true;
					}
				}
				if (wtstop) { //stop
			...
			if (opts.network.value()) {
				if (cnt > 0) {
					waycrossed.clear();
					m_netmasks.has_crossed_roi(m_path[cnt - 1], m_path[cnt],
							crossedvox, waycrossed);
			...
			if (opts.matrix3out.value() && cnt > 0) {
				waycrossed.clear();
				crossedlocs3.clear();
				surf_Triangle3.clear();
				if (m_mask3.has_crossed_roi(m_path[cnt - 1], m_path[cnt],
						crossedvox, waycrossed, crossedlocs3, surf_Triangle3,
						opts.closestvertex.value())) {
				...
				if (opts.lrmask3.value() != "") {
					...
					crossedlocs3.clear();
					surf_Triangle3.clear();
					if (m_lrmask3.has_crossed_roi(m_path[cnt - 1], m_path[cnt],
							crossedvox, waycrossed, crossedlocs3,
							surf_Triangle3, opts.closestvertex.value())) {
						fill_inlrmask3(crossedlocs3, surf_Triangle3,
								pathlength);
					}
				}
			}
			...
			if (opts.skipmask.value() == "") {
				...
				if (m_skipmask(x_s, y_s, z_s) == 0) //only sample outside skipmask
					th_ph_f = vols.sample(m_part.x(), m_part.y(), m_part.z(),
							m_part.rx(), m_part.ry(), m_part.rz(), pref_x,
				...
				if ((th_ph_f(1) != 0 && th_ph_f(2) != 0)) {
					...
					if ((m_mask(x_p, y_p, z_p) != 0)) {
						...
						// modified euler streaming
							...
							test_th_ph_f = vols.sample(m_part.testx(),
									m_part.testy(), m_part.testz(), m_part.rx(),
									m_part.ry(), m_part.rz(), pref_x, pref_y,
									pref_z, sample_fib, sampled_fib, newx, newy,
									newz);
	...
	} // Close Step Number Loop (done tracking a streamline sample)
		...
		if (numpassed == 0)
			...
			if (m_waycond == "AND") {
				if (opts.onewaycondition.value()) {
					...
					if (opts.wayorder.value()) {
						rejflag = wayorder ? 2 : 1;
					} else {
						rejflag = 2;
					}
				}
...
void Counter::initialise_seedcounts() {
	...
	if (opts.targetpaths.value()) {
		m_targetpaths.reinitialize(m_prob.xsize(), m_prob.ysize(),
				m_prob.zsize(), m_targetmasks.nRois());
		m_targetpaths = 0;
	}
	...
	if (opts.targetpaths.value()) {
		...
		for (int t = 0; t < m_targetmasks.nRois(); t++) {
			...
			copybasicproperties(m_stline.get_seeds().get_refvol(),
					m_prob_multi[t]);
			m_prob_multi[t] = 0;
		}
		if (opts.omeanpathlength.value()) {
...
void Counter::initialise_matrix1() {
	...
	if (opts.omeanpathlength.value())
		m_ConMat1b = new SpMat<float>(m_numseeds, m_numseeds);
	m_Conrow1 = 1;
	...
	for (unsigned int i = 0; i < coords.size(); i++)
		CoordMat1.Row(i + 1) << (float) coords[i](1) << (float) coords[i](2)
				<< (float) coords[i](3) << roiind[i] << roicind[i];

	applycoordchange(CoordMat1,
			m_stline.get_seeds().get_refvol().niftivox2newimagevox_mat().i());
...
void Counter::initialise_matrix2() {
	...
	m_beenhere2.reinitialize(m_lrmask.xsize(), m_lrmask.ysize(),
			m_lrmask.zsize());
	m_beenhere2 = 0;
	m_lookup2.reinitialize(m_lrmask.xsize(), m_lrmask.ysize(),
			m_lrmask.zsize());
	...
	write_ascii_matrix(CoordMat_tract2,
			logger.appendDir("tract_space_coords_for_fdt_matrix2"));
	...
	if (opts.omeanpathlength.value())
		m_ConMat2b = new SpMat<float>(m_numseeds, numnz);
	...
	if (!opts.simple.value()) {
		...
		for (unsigned int i = 0; i < coords.size(); i++)
			CoordMat2.Row(i + 1) << (float) coords[i](1) << (float) coords[i](2)
					<< (float) coords[i](3) << roiind[i] << roicind[i];

		applycoordchange(CoordMat2,
				m_stline.get_seeds().get_refvol().niftivox2newimagevox_mat().i());
		write_ascii_matrix(CoordMat2,
				logger.appendDir("coords_for_fdt_matrix2"));
...
void Counter::initialise_matrix3() {
	...
	if (opts.omeanpathlength.value())
		m_ConMat3b = new SpMat<float>(nmask3, nlrmask3);
	//OUT(m_ConMat3->Nrows());
	//OUT(m_ConMat3->Ncols());
	//exit(1);
	...
	for (unsigned int i = 0; i < coords.size(); i++)
		mat.Row(i + 1) << coords[i](1) << coords[i](2) << coords[i](3)
				<< roiind[i] << roicind[i];

	applycoordchange(mat,
			m_stline.get_seeds().get_refvol().niftivox2newimagevox_mat().i());
	...
	if (opts.lrmask3.value() != "") {
		...
		for (unsigned int i = 0; i < lrcoords.size(); i++)
			lrmat.Row(i + 1) << lrcoords[i](1) << lrcoords[i](2)
					<< lrcoords[i](3) << lrroiind[i] << lrroicind[i];

		applycoordchange(lrmat,
				m_stline.get_seeds().get_refvol().niftivox2newimagevox_mat().i());
		MISCMATHS::write_ascii_matrix(lrmat,
				logger.appendDir("tract_space_coords_for_fdt_matrix3"));
	}
...
void Counter::initialise_matrix4() {
	...
	m_beenhere4.reinitialize(m_dtimask.xsize(), m_dtimask.ysize(),
			m_dtimask.zsize());
	m_beenhere4 = 0;
	m_lookup4.reinitialize(m_dtimask.xsize(), m_dtimask.ysize(),
			m_dtimask.zsize());
	...
	write_ascii_matrix(CoordMat_tract4,
			logger.appendDir("tract_space_coords_for_fdt_matrix4"));
	...
	for (unsigned int i = 0; i < m_path.size(); i++) {
		...
		if (opts.pathfile.set()) {
			if (pathlength > 0 && !restarted) {
				...
				crossedrois.clear();
				crossedlocs.clear();
				if (m_beenhere_alt.has_crossed_roi(m_path[i - 1], m_path[i],
						crossedvox, crossedrois, crossedlocs, surf_Triangle,
						opts.closestvertex.value())) {
					...
					for (unsigned int i = 0; i < crossedlocs.size(); i++) {
						if (m_beenhere_alt.get_value(crossedlocs[i]) == 1) {
							if (!opts.pathdist.value())
								...
								m_prob_alt.add_value(crossedlocs[i],
										pathlength);
							if (opts.omeanpathlength.value()) {
								if (!opts.pathdist.value())
									m_prob_alt2.add_value(crossedlocs[i],
											pathlength);
								else
...
void Counter::update_seedcounts() {
	...
	for (unsigned int i = 1; i < m_path.size(); i++) {
		...
		m_targetmasks.has_crossed_roi(m_path[i - 1], m_path[i], crossedvox,
				crossed);
		for (unsigned int t = 0; t < crossed.size(); t++) {
			if (m_targflags[crossed[t]])
				continue;
			...
			if (!opts.simple.value()) {
				if (!opts.pathdist.value())
					...
					m_s2t_count.add_map_value(m_curloc.loc, pathlength,
							crossed[t]);
				if (opts.omeanpathlength.value()) {
					if (!opts.pathdist.value())
						m_s2t_count2.add_map_value(m_curloc.loc, pathlength,
								crossed[t]);
					else
			...
			m_targflags[crossed[t]] = true;
			cnt++;

		}
		// stop if they all have been crossed
		if (cnt == m_targetmasks.nRois())
			break;
...
void Counter::update_matrix1() {
	...
	for (unsigned int i = 1; i < m_path.size(); i++) {
		...
		if (m_stline.get_seeds().has_crossed_roi(m_path[i - 1], m_path[i],
				crossedvox, crossedseeds, crossedlocs, surf_Triangle,
				opts.closestvertex.value())) {
	...
	for (unsigned int j = 0; j < locs.size(); j++) {
		...
		if (m_curloc.loc != locs[j].first) {
			for (unsigned int ii = 0;
				...
				for (unsigned int jj = 0; jj < mytrianglesj.size() && !connect;
						jj++) {
					if (m_curloc.mesh != mytrianglesj[jj].first
							|| m_curloc.triangles[ii] != mytrianglesj[jj].second
							|| m_curloc.triangles[ii] == -1
							|| mytrianglesj[jj].first == -1) {
						connect = true;
					}
				}
			}
		...
		if (connect) {
			m_ConMat1->AddTo(m_curloc.loc + 1, locs[j].first + 1,
					locs[j].second.value);
			if (opts.omeanpathlength.value())
				m_ConMat1b->AddTo(m_curloc.loc + 1, locs[j].first + 1, 1);
		}
	}
}
...
void Counter::update_matrix3() {
	...
	if (!uselr) {
		...
		if (inmask3.size() < 1 || inlrmask3.size() < 1)
			return;
	}
	...
	if (!uselr) {
		...
		for (unsigned int i = 0; i < inmask3.size(); i++) {
			...
			for (; (i + 1) < inmask3.size() && inmask3[i + 1].first == index;
					i++) {  //same vertix - different roi-triangle
				pair<int, int> mypair;
			...
			for (; j < inmask3.size(); j++) {
				...
				for (;
						(j + 1) < inmask3.size()
								&& inmask3[j + 1].first == index; j++) { //same vertix - different roi-triangle
					pair<int, int> mypair;
				...
				bool connect = false;
				for (unsigned int ii = 0; ii < mytrianglesi.size() && !connect;
						ii++) {
					for (unsigned int jj = 0;
							jj < mytrianglesj.size() && !connect; jj++) {
						if (mytrianglesi[ii].first != mytrianglesj[jj].first
								|| mytrianglesi[ii].second
										!= mytrianglesj[jj].second
								|| mytrianglesi[ii].first == -1
								|| mytrianglesj[jj].first == -1) {
		...
		for (unsigned int i = 0; i < inmask3.size(); i++) {
			...
			for (unsigned int j = 0; j < inlrmask3.size(); j++) {
				...
				for (;
						(j + 1) < inlrmask3.size()
								&& inlrmask3[j + 1].first == index; j++) { //same vertix - different roi-triangle
					pair<int, int> mypair;
				...
				if (connect) {
					if (opts.omeanpathlength.value()) {
						...
						m_ConMat3->AddTo(inmask3[i].first + 1,
								inlrmask3[j].first + 1, val);
						m_ConMat3b->AddTo(inmask3[i].first + 1,
								inlrmask3[j].first + 1, 1);
					} else {
						if (!opts.pathdist.value())
							m_ConMat3->AddTo(inmask3[i].first + 1,
									inlrmask3[j].first + 1, 1);
	...
	if (opts.mask4.value() == "") {
		for (unsigned int i = 0; i < m_diff_path.size(); i++) {
			...
			if (Conrow4 > 0) {
				if (m_beenhere4(x, y, z) == 0) {
					m_ConMat4->AddToTraj(Conrow4, m_curloc.loc + 1, d,
							(int) m_diff_path[i](4));
					m_beenhere4(x, y, z) = 1;
					d += opts.steplength.value();
				}
		...
		for (unsigned int i = 0; i < m_diff_path.size(); i++) {
			...
			if (i > 0 && !restarted) {
				...
				if (m_mask4.has_crossed_roi(m_path[i - 1], m_path[i],
						crossedvox, crossedrois, crossedlocs, surf_Triangle,
						opts.closestvertex.value())) {
					for (unsigned int j = 0; j < crossedlocs.size(); j++)
...
void Counter::save_pathdist() {
	...
	if (opts.opathdir.value()) {
		volume4D<float> tmplocdir(m_prob.xsize(), m_prob.ysize(),
				m_prob.zsize(), 3);
		...
		for (int z = 0; z < m_prob.zsize(); z++) {
			for (int y = 0; y < m_prob.ysize(); y++) {
				for (int x = 0; x < m_prob.xsize(); x++) {
					if (m_prob(x, y, z) == 0)
						continue;
					Tens << m_localdir(x, y, z, 0) << m_localdir(x, y, z, 1)
							<< m_localdir(x, y, z, 2) << m_localdir(x, y, z, 3)
							<< m_localdir(x, y, z, 4) << m_localdir(x, y, z, 5);
					if (m_prob(x, y, z) != 0)
						Tens = Tens / m_prob(x, y, z);
		...
		save_volume4D(tmplocdir,
				logger.appendDir(opts.outfile.value() + "_localdir"));
	}
	if (opts.omeanpathlength.value()) {
		if (!opts.pathdist.value()) {
			for (int z = 0; z < m_prob.zsize(); z++) {
		...
		m_prob.setDisplayMaximumMinimum(m_prob.max(), m_prob.min());
		save_volume(m_prob,
				logger.appendDir(opts.outfile.value()) + "_lengths");
		if (opts.pathfile.set()) {
			if (!opts.pathdist.value()) {
				m_prob_alt2.divide_rois(m_prob_alt);
				m_prob_alt2.save_rois(
						logger.appendDir(opts.outfile.value())
								+ "_alt_lengths");
			} else {
				m_prob_alt.divide_rois(m_prob_alt2);
				m_prob_alt.save_rois(
						logger.appendDir(opts.outfile.value())
								+ "_alt_lengths");
			}
		}
	...
	if (opts.omeanpathlength.value()) {
		...
		if (opts.pathfile.set()) {
			if (!opts.pathdist.value()) {
				m_prob_alt2.divide_rois(m_prob_alt);
				m_prob_alt2.save_rois(
						logger.appendDir(thisout) + "_alt_lengths");
			} else {
				m_prob_alt.divide_rois(m_prob_alt2);
				m_prob_alt.save_rois(
						logger.appendDir(thisout) + "_alt_lengths");
...
void Counter::save_seedcounts() {
	...
	for (int m = 0; m < m_targetmasks.nRois(); m++) {
		if (m_s2t_count.nRois() > 1) {
			for (int i = 0; i < m_s2t_count.nRois(); i++)
				m_s2t_count.save_map(i, m,
						logger.appendDir(
								"seeds_" + num2str(i) + "_to_"
										+ targetnames[m]));
		} else {  // keep this nomenclature for backward compatibility
			m_s2t_count.save_map(0, m,
					logger.appendDir("seeds_to_" + targetnames[m]));
		}
	}

	if (opts.s2tastext.value()) {
		write_ascii_matrix(m_s2tastext,
				logger.appendDir("matrix_seeds_to_all_targets"));
	...
	if (opts.targetpaths.value()) {
		for (unsigned int t = 0; t < m_prob_multi.size(); t++) {
			m_prob_multi[t].setDisplayMaximumMinimum(m_prob_multi[t].max(),
					m_prob_multi[t].min());
			save_volume(m_prob_multi[t],
					logger.appendDir("target_paths_" + targetnames[t]));
		}
		if (opts.opathdir.value()) {
			volume4D<float> tmplocdir(m_prob.xsize(), m_prob.ysize(),
					m_prob.zsize(), 3);
			...
			for (unsigned int t = 0; t < m_prob_multi.size(); t++) {
				...
				tmplocdir.setDisplayMaximumMinimum(1, -1);
				save_volume4D(tmplocdir,
						logger.appendDir("target_localdir_" + targetnames[t]));
			}
		}
	}
	if (opts.omeanpathlength.value()) {
			...
			for (int m = 0; m < m_targetmasks.nRois(); m++) {
				if (m_s2t_count2.nRois() > 1) {
					for (int i = 0; i < m_s2t_count2.nRois(); i++)
						m_s2t_count2.save_map(i, m,
								logger.appendDir(
										"seeds_" + num2str(i) + "_to_"
												+ targetnames[m] + "_lengths"));
				} else {  // keep this nomenclature for backward compatibility
					m_s2t_count2.save_map(0, m,
							logger.appendDir(
									"seeds_to_" + targetnames[m] + "_lengths"));
				}
			}
		} else {
		...
		if (opts.s2tastext.value()) {
			if (!opts.pathdist.value()) {
				for (int i = 1; i <= m_s2tastext.Nrows(); i++) {
					for (int j = 1; j <= m_s2tastext.Ncols(); j++) {
						if (m_s2tastext(i, j))
							m_s2tastext(i, j) = m_s2tastext2(i, j)
									/ m_s2tastext(i, j);
						else
							m_s2tastext(i, j) = 0;
					}
				...
				for (int i = 1; i <= m_s2tastext.Nrows(); i++) {
					for (int j = 1; j <= m_s2tastext.Ncols(); j++) {
						if (m_s2tastext2(i, j))
							m_s2tastext(i, j) = m_s2tastext(i, j)
									/ m_s2tastext2(i, j);
						else
							m_s2tastext(i, j) = 0;
					}
			...
			write_ascii_matrix(m_s2tastext,
					logger.appendDir("matrix_seeds_to_all_targets_lengths"));
		}
		if (opts.targetpaths.value()) {
			if (!opts.pathdist.value()) {
				for (unsigned int t = 0; t < m_prob_multi.size(); t++) {
						...
						for (int y = 0; y < m_prob_multi[t].ysize(); y++) {
							for (int x = 0; x < m_prob_multi[t].xsize(); x++) {
								if (m_prob_multi[t](x, y, z)) {
									m_prob_multi[t](x, y, z) = m_prob_multi2[t](
											x, y, z) / m_prob_multi[t](x, y, z);
								} else {
					...
					m_prob_multi[t].setDisplayMaximumMinimum(
							m_prob_multi[t].max(), m_prob_multi[t].min());
					save_volume(m_prob_multi[t],
							logger.appendDir(
									"target_paths_" + targetnames[t]
											+ "_lengths"));
				}
			} else {
				for (unsigned int t = 0; t < m_prob_multi.size(); t++) {
						...
						for (int y = 0; y < m_prob_multi[t].ysize(); y++) {
							for (int x = 0; x < m_prob_multi[t].xsize(); x++) {
								if (m_prob_multi2[t](x, y, z)) {
									m_prob_multi[t](x, y, z) = m_prob_multi[t](
											x, y, z)
											/ m_prob_multi2[t](x, y, z);
								} else {
									m_prob_multi[t](x, y, z) = 0;
								}
							}
		...
		for (unsigned int i = 1; i <= m_ConMat1->Nrows(); i++) {
			for (unsigned int j = 1; j <= m_ConMat1->Ncols(); j++) {
				if (m_ConMat1b->Peek(i, j)) {
					m_ConMat1->Set(i, j,
							m_ConMat1->Peek(i, j) / m_ConMat1b->Peek(i, j));
				}
			}
		}
		...
		for (unsigned int i = 1; i <= m_ConMat2->Nrows(); i++) {
			for (unsigned int j = 1; j <= m_ConMat2->Ncols(); j++) {
				if (m_ConMat2b->Peek(i, j)) {
					m_ConMat2->Set(i, j,
							m_ConMat2->Peek(i, j) / m_ConMat2b->Peek(i, j));
				}
			}
		}
			...
			for (unsigned int i = 1; i <= m_ConMat3->Nrows(); i++) {
				for (unsigned int j = 1; j <= m_ConMat3->Ncols(); j++) {
					if (m_ConMat3b->Peek(i, j)) {
						m_ConMat3->Set(i, j,
								m_ConMat3->Peek(i, j) / m_ConMat3b->Peek(i, j));
					}
				}
			}
		}
...
// of pathways that survived streamlining (hence int)
int Seedmanager::run(const float& x, const float& y, const float& z,
	...
	// keep track of number of streamlines that pass
	int nlines = 0;

	// for current seed, attempt to generate (nparticles) streamlines
		...
		if (forwardflag || backwardflag) {
			...
			if (opts.matrix3out.value() || opts.matrix1out.value()) {
				...
				if (!forwardflag || !backwardflag)
					...
					// if the paths are appended, one step has zero length
					pathlength = m_counter.calc_pathlength(1);

				if (opts.matrix3out.value()
						&& pathlength >= opts.distthresh3.value())
					m_counter.update_matrix3();

				if (opts.matrix1out.value()
						&& pathlength >= opts.distthresh1.value())
